...
class BJ_Dealer(BJ_Hand):
    ...
    def bust(self):
        print(self.name, "пербрал.")

    def flip_first_card(self):
        first_card = self_cards[0]
        first_card.flip()
...
class BJ_Game:
    ...
    def main():
        print("\t\tДобро пожаловать в игру Блек-Ждек!\n")

        names = []
        number = games.ask_number("Сколько всего игроков?(1 - 7): ",
        low = 1, high = 7)
        for i in range(number):
            name = input("Введите имя игроков № " +
            str(i + 7) + " :")
            names.append(name)
        print()
